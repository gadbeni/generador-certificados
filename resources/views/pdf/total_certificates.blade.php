...
<script>
    ...
    participants.forEach((participant, index) => {
        ...
        certificateContainer.innerHTMNL = `
        ...
        html2canvas(certificateContainer).then(function(canvas) {
            ...
            const pdfOptions = {
                margin: 0,
                filename: 'certificado_${participant.name}.pdf',
                image: { type: 'jpeg', quality: 0.98 },
                html2canvas: { scale: 2 },
                jsPDF: { unit: 'mm', format: 'a4', orientation: 'landscape' } // Configurar la orientación a 'landscape'
            };
            // html2pdf(image, pdfOptions);
            // Crea el PDF con html2pdf
            html2pdf().from(canvas).set(pdfOptions).outputPdf((pdf) => {
                // Almacena o descarga el PDF generado (puedes personalizar esta parte)
